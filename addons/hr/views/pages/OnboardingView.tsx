...
const OnboardingView: React.FC = () => {
  ...
  const [title, setTitle] = useState('Créer le compte email');
  ...
  const load = async (empId?: string) => {
    setLoading(true);
    try { setTasks(await taskService.list('onboarding', empId ? Number(empId) : undefined)); } finally { setLoading(false); }
  };
  ...
  return (
    <HrLayout>
      <div>
        ...
        <div className="mb-6">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={async () => {
              ...
              const created = await taskService.create({ kind: 'onboarding', title, employee_id: employeeId ? Number(employeeId) : undefined });
        ...
        <Card>
          <CardHeader>
            <CardTitle>Checklist d'accueil</CardTitle>
          </CardHeader>
          <CardContent>
