...
export const cacheManifests = async (manifests: AddonManifest[]): Promise<void> => {
  try {
    ...
    await Promise.all(manifests.map(async manifest => {
      const ttl = 3600; // TTL par défaut de 1h pour tous les modules
  ...
  const manifests = await Promise.all(
    moduleDirs.map(async dir => {
      try {
        ...
        const manifest = (await import(/* @vite-ignore */ manifestPath)).default;
        return { ...manifest, name: dir };
      } catch (error) {
        warn(`Failed to load manifest for ${dir}:`, error);
        return null;
      }
    })
  );
...
export const watchModules = (onChange?: () => void) => {
  ...
  watcher.on('change', path => {
    info(`Manifest changed: ${path}`);
    redis.del('modules:manifests');
    onChange?.();
  });

  watcher.on('unlink', path => {
    info(`Manifest removed: ${path}`);
    redis.del('modules:manifests');
    onChange?.();
  });

  watcher.on('add', path => {
    info(`New manifest detected: ${path}`);
    redis.del('modules:manifests');
    onChange?.();
  });
